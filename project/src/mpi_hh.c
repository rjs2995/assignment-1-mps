...
int main( int argc, char **argv )
{
  ...
  sprintf( graph_fname, "graphs/p1d%dc%d_%s.png",
		size, num_dendrs, num_comps, time_str );
  ...
  for (t_ms = 1; t_ms < COMPTIME; t_ms++) {
	...
	for (step = 0; step < STEPS; step++) {
	  ...
	  if(rank == 0){ 
		for(i = 1; i < size; i++){
			MPI_Recv(&mpi_buffer, 1, MPI_DOUBLE, i, msgtag, MPI_COMM_WORLD, &status);
			soma_params[2] += current;		
		}

	  }
	  else{ 
		MPI_Send(&soma_params[2], 1, MPI_DOUBLE, 0, msgtag, MPI_COMM_WORLD);
	  }
	  ...
	  if(rank == 0){
	  	...
	  	for(i = 1; i < size; i++){
			MPI_Send(&y[0], 1, MPI_DOUBLE, i, msgtag_2, MPI_COMM_WORLD);
		}
	  }
          else{
		MPI_Recv(&y[0], 1, MPI_DOUBLE, 0, msgtag_2, MPI_COMM_WORLD, &status);
	  }
